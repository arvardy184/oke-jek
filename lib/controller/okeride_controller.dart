...
PackageInfo packageInfo = await PackageInfo.fromPlatform();
    ...
    try {
      ...
      print(
        "data di okeride controller order/new ${data.toString()}",
      );
      ...
      // lo("res di okeride controller order/new $response");
      debugPrint("res di okeride controller order/new $response");
  ...
  void checkPrice() async {
    ...
    try {
      ...
      debugPrint("res di okeride controller order/calculate $response");
  ...
  void getCouponCode(String couponCode) async {
    ...
    try {
      ...
      if (baseResponse.data.coupon!.id == 0) {
        print(baseResponse.data.coupon!.id);
...
void getNearestDriver() async {
  ...
  Future.delayed(Duration(seconds: 1), () async {
    ...
    try {
      ...
      debugPrint("res di okeride controller drivers-locations $response");

      var responseBody = response.data;

      // Check if response body or required data is null/empty
      if (responseBody == null || 
          responseBody['data'] == null || 
          responseBody['data']['driver_locations'] == null ||
          responseBody['data']['driver_locations'].isEmpty) {
        isLoading.value = false;
        // Optional: Show a message to user that no drivers are available
        Fluttertoast.showToast(
          msg: 'Tidak ada driver yang tersedia di sekitar Anda saat ini',
          fontSize: 12,
      ...
      // If we have valid data, proceed with adding locations and markers
      ...
      if (driverLocation.isNotEmpty) {
        Fluttertoast.showToast(
          msg: 'Ditemukan ${driverLocation.length} driver di sekitar Anda',
          fontSize: 12
        );
      }
      
    } on DioException catch (e) {
      print('Error getting nearest driver: ${e.message}');
      isLoading.value = false;
      Fluttertoast.showToast(
        msg: 'Gagal mendapatkan data driver. Silakan coba lagi.',
        fontSize: 12,
        backgroundColor: Colors.red,
        textColor: Colors.white
      );
