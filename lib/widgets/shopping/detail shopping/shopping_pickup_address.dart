...
class ShoppigPickUpAddress extends StatelessWidget {
  ...
  Widget _buildAddressItem({
    ...
    required RxString detail,
  }) {
    return Row(
      children: [
        Icon(icon, color: OkejekTheme.primary_color),
        SizedBox(width: 16),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              Obx(() => Text(address.value, style: TextStyle(fontSize: 12))),
              Obx(() => Text(detail.value, style: TextStyle(fontSize: 12, color: Colors.grey))),
            ],
          ),
        ),
      ],
